...
if [ "$#" -eq 1 ] && [ "$1" == "-L" ]; then
	...
	'
    # 输出IP地址数量
    # ip_count=$(echo "$nft_output" | awk '/elements = \{/,/\}/' | grep -oP '(\d{1,3}\.){3}\d{1,3}\/\d+|([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\/\d+' | wc -l) #仅支持IPV4
    ...
    echo "Members:"
    # 显示元素中的IP/网段
    # echo "$nft_output" | awk '/elements = \{/,/\}/' | grep -oP '(\d{1,3}\.){3}\d{1,3}\/\d+|([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\/\d+' | sort -u #仅支持IPV4
